...
char *YOLOv562Detector::readLine(FILE *fp, char *buffer, int *len)
{
    int ch;
    int i = 0;
    size_t buff_len = 0;
...
int YOLOv562Detector::readLines(const char *fileName, char *lines[], int max_line)
{
...
int YOLOv562Detector::loadLabelName(const char *locationFilename, char *label[])
{
                                   ...
                                   detect_result_group_t *group)
{
    ...
    for (int i = 0; i < validCount; ++i)
    {
        ...
        if ((bottom - top) <= 2)
            continue;
        //------------------------------------------------------------------------------------------------------------------------
        group->results[last_count].box.x = (clamp(x1, 0, model_in_w) / scale_w);
        group->results[last_count].box.y = (clamp(y1, 0, model_in_h) / scale_h);
        group->results[last_count].box.width = (clamp(x2, 0, model_in_w) / scale_w) - group->results[last_count].box.x;
        group->results[last_count].box.height = (clamp(y2, 0, model_in_h) / scale_h) - group->results[last_count].box.y;

        group->results[last_count].confidence = obj_conf;
