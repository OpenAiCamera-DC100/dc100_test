...
static void *GetMediaBuffer(void *arg) {
  ...
  while (g_flag_run) {
    buffer = DC_MPI_SYS_GetMediaBuffer(DC_ID_VI, RK_NN_INDEX, -1);
    if (!buffer) {
      continue;
    }
    // printf("Get Frame:ptr:%p, fd:%d, size:%zu, mode:%d, channel:%d, "
    //        "timestamp:%lld\n",
    //        DC_MPI_MB_GetPtr(buffer), DC_MPI_MB_GetFD(buffer),
    //        DC_MPI_MB_GetSize(buffer),
    //        DC_MPI_MB_GetModeID(buffer), DC_MPI_MB_GetChannelID(buffer),
    //        DC_MPI_MB_GetTimestamp(buffer));
    ...
    nv12_to_rgb24(DC_MPI_MB_GetPtr(buffer), rga_buffer,
...
static void *MainStream() {
  ...
  while (g_flag_run) {
    buffer = DC_MPI_SYS_GetMediaBuffer(
        DC_ID_VI, cfg.session_cfg[DRAW_INDEX].stViChn.s32ChnId, -1);
    if (!buffer)
      continue;
    // draw
    if (rknn_list_size(rknn_list_)) {
      ...
      for (int j = 0; j < detect_result_group.count; j++) {
        ...
        nv12_border((char *)DC_MPI_MB_GetPtr(buffer),
    ...
    DC_MPI_SYS_SendMediaBuffer(
        DC_ID_VENC, cfg.session_cfg[DRAW_INDEX].stVenChn.s32ChnId, buffer);
    DC_MPI_MB_ReleaseBuffer(buffer);
  }
...
static int load_cfg(const char *cfg_file) {
  ...
  while (fgets(line, sizeof(line) - 1, fp)) {
    ...
    if (cfg.session_cfg[count].video_type != DC_CODEC_TYPE_NONE) {
      count++;
    } else {
...
static void SAMPLE_COMMON_VI_Start(struct Session *session,
                                   VI_CHN_WORK_MODE mode, DC_S32 vi_pipe) {
  ...
  DC_MPI_VI_SetChnAttr(vi_pipe, session->stViChn.s32ChnId, &vi_chn_attr);
  DC_MPI_VI_EnableChn(vi_pipe, session->stViChn.s32ChnId);
}
...
static void SAMPLE_COMMON_VENC_Start(struct Session *session) {
  ...
  DC_MPI_VENC_CreateChn(session->stVenChn.s32ChnId, &venc_chn_attr);
}

static DC_CHAR optstr[] = "?::a::c:b:l:p:I:M:";
...
int main(int argc, char **argv) {
  DC_CHAR *pCfgPath = "/oem/usr/share/rtsp-nn.cfg";
  ...
  for (int i = 0; i < cfg.session_count; i++) {
    ...
    // VI create
    printf("VI create\n");
    cfg.session_cfg[i].stViChn.enModId = DC_ID_VI;
    cfg.session_cfg[i].stViChn.s32ChnId = i;
    SAMPLE_COMMON_VI_Start(&cfg.session_cfg[i], VI_WORK_MODE_NORMAL, s32CamId);
    // VENC create
    printf("VENC create\n");
    cfg.session_cfg[i].stVenChn.enModId = DC_ID_VENC;
    cfg.session_cfg[i].stVenChn.s32ChnId = i;
    SAMPLE_COMMON_VENC_Start(&cfg.session_cfg[i]);
    if (i == DRAW_INDEX)
      DC_MPI_VI_StartStream(s32CamId, cfg.session_cfg[i].stViChn.s32ChnId);
    else
      DC_MPI_SYS_Bind(&cfg.session_cfg[i].stViChn,
  ...
  while (g_flag_run) {
    for (int i = 0; i < cfg.session_count; i++) {
      ...
      // send video buffer
      buffer = DC_MPI_SYS_GetMediaBuffer(
          DC_ID_VENC, cfg.session_cfg[i].stVenChn.s32ChnId, 0);
      if (buffer) {
        rtsp_tx_video(cfg.session_cfg[i].session, DC_MPI_MB_GetPtr(buffer),
                      DC_MPI_MB_GetSize(buffer),
                      DC_MPI_MB_GetTimestamp(buffer));
        DC_MPI_MB_ReleaseBuffer(buffer);
      }
  ...
  for (int i = 0; i < cfg.session_count; i++) {
    if (i != DRAW_INDEX)
      DC_MPI_SYS_UnBind(&cfg.session_cfg[i].stViChn,
                        &cfg.session_cfg[i].stVenChn);
    DC_MPI_VENC_DestroyChn(cfg.session_cfg[i].stVenChn.s32ChnId);
    DC_MPI_VI_DisableChn(s32CamId, cfg.session_cfg[i].stViChn.s32ChnId);
  }
