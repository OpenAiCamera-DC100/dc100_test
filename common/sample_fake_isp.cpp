...
dc_aiq_sys_ctx_t *aiq_fake_init(dc_aiq_working_mode_t mode, const char *iq_dir,
                                dc_aiq_raw_prop_t prop,
                                void (*callback)(void *)) {
  dc_aiq_sys_ctx_t *ctx = NULL;
  printf("sensor_name is FakeCamera\n");
  ctx = dc_aiq_uapi_sysctl_init("FakeCamera", iq_dir, NULL, NULL);
  if (!ctx) {
    printf("%s: dc_aiq_uapi_sysctl_init fail!\n", __func__);
    return NULL;
  }
  dc_aiq_uapi_sysctl_prepareRkRaw(ctx, prop);
  if (dc_aiq_uapi_sysctl_prepare(ctx, AIQ_WIDTH, AIQ_HEIGHT, mode)) {
    printf("%s: dc_aiq_uapi_sysctl_prepare fail!\n", __func__);
    dc_aiq_uapi_sysctl_deinit(ctx);
    return NULL;
  }
  dc_aiq_uapi_sysctl_registRkRawCb(ctx, callback);
  if (dc_aiq_uapi_sysctl_start(ctx) < 0) {
    printf("%s: dc_aiq_uapi_sysctl_start fail!\n", __func__);
    dc_aiq_uapi_sysctl_deinit(ctx);
    return NULL;
  }

  ahdr_attrib_t hdr_attr;
  dc_aiq_user_api_ahdr_GetAttrib(ctx, &hdr_attr);
  hdr_attr.opMode = HDR_OpMode_Tool;
  for (int i = 0; i < CALIBDB_MAX_MODE_NUM; i++)
    hdr_attr.stTool.tmo.en[i].en = 0;
  dc_aiq_user_api_ahdr_SetAttrib(ctx, hdr_attr);

  return ctx;
}

void aiq_fake_exit(dc_aiq_sys_ctx_t *ctx) {
  if (ctx) {
    dc_aiq_uapi_sysctl_stop(ctx, false);
    dc_aiq_uapi_sysctl_deinit(ctx);
    ctx = NULL;
  }
}
