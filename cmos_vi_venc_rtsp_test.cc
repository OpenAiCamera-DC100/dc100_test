...
static rtsp_session_handle g_rtsp_session_a;
#ifdef DUAL
static rtsp_session_handle g_rtsp_session_b;
#endif //DUAL
...
void video_packet_cb_a(MEDIA_BUFFER mb) {
    ...
    if (g_rtsplive && g_rtsp_session_a) {
        rtsp_tx_video(g_rtsp_session_a, (uint8_t const*)DC_MPI_MB_GetPtr(mb), DC_MPI_MB_GetSize(mb),
...
#ifdef DUAL
void video_packet_cb_b(MEDIA_BUFFER mb) {
    static DC_S32 packet_cnt = 0;
    size_t buffSize;
    if (quit)
        return;
    ...
    //printf("## CAM B: Get packet-%d, size %zu\n", packet_cnt, buffSize);

    if (g_rtsplive && g_rtsp_session_b) {
        rtsp_tx_video(g_rtsp_session_b, (uint8_t const*)DC_MPI_MB_GetPtr(mb), DC_MPI_MB_GetSize(mb),
                      DC_MPI_MB_GetTimestamp(mb));
        rtsp_do_event(g_rtsplive);
    }
...
int main(int argc, char *argv[]) {
    ...
    while ((c = getopt_long(argc, argv, optstr, long_options, NULL)) != -1) {
        ...
        case 'a':
            ...
            if (tmp_optarg) {
                ...
                pIqfilesPath = (char *)"/oem/etc/iqfiles";
            }
            break;
        case 'w':
    ...
    if (pIqfilesPath) {
        ...
        SAMPLE_COMM_ISP_Init(1, hdr_mode, bMultictx, pIqfilesPath);
    ...
    g_rtsp_session_a = rtsp_new_session(g_rtsplive, "/live/main_stream_a");
#ifdef DUAL
    g_rtsp_session_b = rtsp_new_session(g_rtsplive, "/live/main_stream_b");
#endif

 
    if (enCodecType == DC_CODEC_TYPE_H264) {
        rtsp_set_video(g_rtsp_session_a, RTSP_CODEC_ID_VIDEO_H264, NULL, 0);
#ifdef DUAL
        rtsp_set_video(g_rtsp_session_b, RTSP_CODEC_ID_VIDEO_H264, NULL, 0);
#endif
    } else if (enCodecType == DC_CODEC_TYPE_H265) {
        rtsp_set_video(g_rtsp_session_a, RTSP_CODEC_ID_VIDEO_H265, NULL, 0);
#ifdef DUAL
        rtsp_set_video(g_rtsp_session_b, RTSP_CODEC_ID_VIDEO_H265, NULL, 0);
#endif
    } else {
        printf("not support other type\n");
        return -1;
    }
    rtsp_sync_video_ts(g_rtsp_session_a, rtsp_get_reltime(), rtsp_get_ntptime()); 
#ifdef DUAL
    rtsp_sync_video_ts(g_rtsp_session_b, rtsp_get_reltime(), rtsp_get_ntptime()); 
#endif
...
#ifdef DUAL
    ret = DC_MPI_VI_SetChnAttr(1, 1, &vi_chn_attr);
    ret |= DC_MPI_VI_EnableChn(1, 1);
    if (ret) {
...
#ifdef DUAL
    ret = DC_MPI_VENC_CreateChn(1, &venc_chn_attr);
    if (ret) {
    ...
    stEncChn.s32DevId = 0;
    stEncChn.s32ChnId = 0;
    ret = DC_MPI_SYS_RegisterOutCb(&stEncChn, video_packet_cb_a); 
    if (ret) {
    ...
    stEncChn.s32DevId = 1;
    stEncChn.s32ChnId = 1;
    ret = DC_MPI_SYS_RegisterOutCb(&stEncChn, video_packet_cb_b); 
    if (ret) {
    ...
    ret = DC_MPI_SYS_Bind(&stSrcChn, &stDestChn);
    if (ret) {
        printf("ERROR: Bind VI[1] and VENC[1] error! ret=%d\n", ret);
        return 0;
    }
#endif
